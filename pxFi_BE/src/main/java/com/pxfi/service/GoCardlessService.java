...
public class GoCardlessService {
    ...
    public String fetchAccessToken() throws Exception {
        ...
        GoCardlessTokenResponse tokenResponse = objectMapper.readValue(response.getBody(), GoCardlessTokenResponse.class);
        return tokenResponse.getAccessToken();
    }

    public List<Map<String, Object>> getInstitutions(String accessToken, String countryCode) {
        String url = UriComponentsBuilder
            .fromHttpUrl("https://bankaccountdata.gocardless.com/api/v2/institutions/")
    ...
    public EndUserAgreementResponse createEndUserAgreement(String institutionId, String accessToken) throws Exception {
        ...
        EndUserAgreementRequest request = new EndUserAgreementRequest(
            ...
            "90",
            "30",
            List.of("balances", "details", "transactions")
        );
        ...
        if (!response.getStatusCode().is2xxSuccessful()) {
            throw new RuntimeException("Failed to create end-user agreement: " + response.getBody());
        }

        return objectMapper.readValue(response.getBody(), EndUserAgreementResponse.class);
    }

    public RequisitionResponse createRequisition(String accessToken, String institutionId, String agreementId) throws Exception {
    ...
    public RequisitionDetailsResponse getRequisitionDetails(String accessToken, String requisitionId) throws Exception {
        String url = "https://bankaccountdata.gocardless.com/api/v2/requisitions/" + requisitionId + "/";
        ...
        if (!response.getStatusCode().is2xxSuccessful()) {
            throw new RuntimeException("Failed to fetch requisition details: " + response.getBody());
        }

        return objectMapper.readValue(response.getBody(), RequisitionDetailsResponse.class);
    }

    public TransactionsResponse getAccountTransactions(String accessToken, String accountId) throws Exception {
        String url = "https://bankaccountdata.gocardless.com/api/v2/accounts/" + accountId + "/transactions/";
        ...
        if (!response.getStatusCode().is2xxSuccessful()) {
            throw new RuntimeException("Failed to fetch account transactions: " + response.getBody());
        }
