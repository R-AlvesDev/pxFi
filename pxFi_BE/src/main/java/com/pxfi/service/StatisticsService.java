...
public class StatisticsService {
    ...
    public StatisticsService(
            @Lazy TransactionService transactionService, CategoryRepository categoryRepository) {
    ...
    public StatisticsResponse getMonthlyStatistics(String accountId, int year, int month) {
        ...
        List<Transaction> transactions =
                transactionService.getTransactionsByAccountId(accountId, startDate, endDate);
        ...
        for (Transaction tx : transactions) {
            ...
            if (amount.compareTo(BigDecimal.ZERO) > 0) {
                ...
                if (categoryToCheck == null || !categoryToCheck.isAssetTransfer()) {
                    ...
                    String categoryName =
                            tx.getCategoryName() != null ? tx.getCategoryName() : "Uncategorized";
        ...
        List<CategorySpending> expensesByCategory =
                expensesByCategoryMap.entrySet().stream()
    ...
    public YearlyStatisticsResponse getYearlyStatistics(String accountId, int year) {
        ...
        Map<Integer, List<Transaction>> transactionsByMonth =
                transactions.stream()
                        .filter(tx -> !tx.isIgnored())
                        .collect(
                                Collectors.groupingBy(
                                        tx ->
                                                LocalDate.parse(tx.getBookingDate())
                                                        .getMonthValue()));
        ...
        for (int month = 1; month <= 12; month++) {
            List<Transaction> monthlyTransactions =
                    transactionsByMonth.getOrDefault(month, List.of());
                ...
                if (tx.getLinkedTransactionId() != null && !processedIds.contains(tx.getId())) {
                    ...
                    if (linkedTx != null) {
                        BigDecimal netExpense =
                                new BigDecimal(tx.getTransactionAmount().getAmount())
                                        .add(
                                                new BigDecimal(
                                                        linkedTx.getTransactionAmount()
                                                                .getAmount()))
                                        .abs();
        ...
        BigDecimal avgMonthlyIncome =
                (monthsWithIncome > 0)
                        ? totalYearlyIncome.divide(
                                new BigDecimal(monthsWithIncome), 2, RoundingMode.HALF_UP)
                        : BigDecimal.ZERO;
        BigDecimal avgMonthlyExpenses =
                (monthsWithExpenses > 0)
                        ? totalYearlyExpenses.divide(
                                new BigDecimal(monthsWithExpenses), 2, RoundingMode.HALF_UP)
                        : BigDecimal.ZERO;
